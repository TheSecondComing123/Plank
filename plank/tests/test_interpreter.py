...
class PlankTest(unittest.TestCase):
    sections = {
        ...
        "dicts and sets": [
            ...
            Case("s <- {1,2}; out <- contains(s, 2)", True),
            Case("lst <- [1,2,3]; remove(lst, 1); out <- lst[1]", 3),
            Case("d <- {'a':1,'b':2}; remove(d, 'a'); out <- len(d)", 1),
            Case("s <- {1}; extend(s, {2,3}); out <- len(s)", 3),
            Case("d <- {'a':1}; extend(d, {'b':2}); out <- d['b']", 2),
            Case("d <- {'a':1,'b':2}; out <- keys(d)[1]", 'b'),
            Case("d <- {'a':1,'b':2}; out <- values(d)[0]", 1),
            Case("d <- {'x':5}; out <- items(d)[0][0]", 'x'),
            Case("out <- len(union({1,2}, {2,3}))", 3),
            Case("out <- len(intersection({1,2}, {2,3}))", 1),
            Case("out <- len(difference({1,2}, {2}))", 1),
        ],
        "functions": [
        ...
        "builtins": [
            ...
            Case("out <- (1..3)[2]", 3),
            Case("out <- contains([1,2,3], 2)", True),
            Case("lst <- [1,3]; insert(lst, 1, 2); out <- lst[1]", 2),
            Case("lst <- [1]; extend(lst, [2,3]); out <- lst[2]", 3),
        ],
        "comments": [
            Case("a <- 1 # assign a\nout <- a", 1),
