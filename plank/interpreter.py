    ...
    def _setup_builtins(self):
        ...
        def b_split(s, sep):
            if not isinstance(s, str) or not isinstance(sep, str):
                raise Exception("Runtime error: split expects two strings")
            return s.split(sep)

        def b_join(lst, sep):
            if not isinstance(lst, list) or not isinstance(sep, str):
                raise Exception("Runtime error: join expects a list and a string")
        ...
        def b_replace(s, old, new):
            if not isinstance(s, str) or not isinstance(old, str) or not isinstance(new, str):
                raise Exception("Runtime error: replace expects three strings")
